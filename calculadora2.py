...
def boton_igual():
    try:
        resultado = str(eval(entrada.get().replace('x', '*').replace('√', 'sqrt').replace('π', 'pi')))
        entrada.delete(0, tk.END)
...
boton_pi = crear_boton("π", 5, 3, lambda: boton_click('π'))
boton_potencia = crear_boton("^", 5, 2, lambda: boton_click('^'))
boton_raiz = crear_boton("√", 5, 1, lambda: boton_click('√'))
boton_factorial = crear_boton("!", 5, 0, lambda: boton_click('!'))
boton_e = crear_boton("e", 6, 3, lambda: boton_click('e'))
boton_porcentaje = crear_boton("%", 6, 2, lambda: boton_click('%'))
boton_log = crear_boton("log", 6, 1, lambda: boton_click('log('))
boton_heart = crear_boton(")", 6, 0, lambda: boton_click('('))
